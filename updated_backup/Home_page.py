...
def design():
    ...
    quote_icon  = PhotoImage(file="images/quote.png")
    client_icon = PhotoImage(file="images/client.png")
    client_details_icon = PhotoImage(file="images/client_details.png")
    project_update_icon = PhotoImage(file="images/proj_upd.png")
    project_time_icon = PhotoImage(file="images/proj_time.png")
    dashboard_icon = PhotoImage(file="images/dashbord.png")
    clientfinance_icon = PhotoImage(file="images/clientsFinance.png")
    companyFinance_icon = PhotoImage(file="images/companyFinance.png")
    userAccount_icon = PhotoImage(file='images/accountSetting.png')
    databaseSetting_icon = PhotoImage(file="images/database.png")


    notebook2.add(quotation_tab, text="Register Service Quotations", image=quote_icon, compound="bottom")
    notebook2.add(registration_tab, text="Client Registration", image=client_icon, compound='bottom')
    notebook2.add(client_details_tab, text="Client Details", image=client_details_icon, compound='bottom')
    notebook2.add(project_update_tab, text="Project update", image=project_update_icon, compound="bottom")
    notebook2.add(project_timeline_tab, text="Project timeline", image=project_time_icon, compound="bottom")
    notebook2.add(dashboard_tab, text="Dashboard", image=dashboard_icon, compound="bottom")
    notebook2.add(client_finances_tab, text="Client finances", image=clientfinance_icon, compound="bottom")
    notebook2.add(company_finances_tab, text="Company Finances",  image=companyFinance_icon, compound="bottom")
    notebook2.add(user_account_setting_tab, text="User Account Settings", image=userAccount_icon, compound="bottom")
    notebook2.add(Database_setting_tab, text="Database Settings", image=databaseSetting_icon, compound="bottom")
    ...
    def update_org():
        ...
        if upd_email_password != upd_repeat_email_passwd:
            ...
            #saving the data to the database for use later on for email
            upd_org_detail = Organization(organization_id=1,
                                          organization_name=upd_org_name,
                                          organization_contact= upd_org_contact, 
                                          location=upd_org_location, 
                                          organization_email=upd_org_email, 
                                          email_password=upd_email_password)
    ...
    def register_employee():
        ...
        if not emp_role:
            ...
            try:
                ...
                clear_emp_entry()
                emp_id_entry.delete(0, tk.END)
                emp_id_entry.insert(0, employee_id())
    ...
    def quotation():
        ...
        def save_quote_func():
            ...
            try:
                ...
                messagebox.showinfo(title="Create quotation", message="Data saved successfully")
            except Exception as e:
                create_session.rollback()
                messagebox.showerror(title="Create quotation", message=f"Error saving data; {e}")
            finally:
...
"""
    ...
    def client_registration():
        ...
        def cl_load_data():
            ...
            if cl_quote:
                ...
                messagebox.showerror(title="Load quotation", message="Error loading, provide correct quotation number")
        ...
        def cl_reg():
            ...
            cl_deadline = end_date.get_date()
            cl_start_date = start_date.get_date()
            ...
            new_client = Client(client_id=cl_id,
                                ...
                                project_deadline=cl_deadline,
                                registration_date=datetime.now(),
                                project_start_date=cl_start_date)
            ...
            try:
                ...
                for client in session.query(Client).all():
                    client_tree.insert("", "end", values=(client.client_id, 
                                                          client.client_name, 
                                                          client.contact,
                                                          client.email, 
        ...
        tk.Label(client_reg_frame, text='Project Start date:').grid(row=11, column=0)
        start_date = DateEntry(client_reg_frame, width=12, background='darkblue', foreground='white', borderwidth=2)
        start_date.grid(row=11, column=1)

        tk.Label(client_reg_frame, text='Project End Date:').grid(row=12, column=0)
        end_date = DateEntry(client_reg_frame, width=12, background='darkblue', foreground='white', borderwidth=2)
        end_date.grid(row=12, column=1, padx=10, pady=5)
    ...
    def project_update():
        ...
        def load_client():
            pr_client_id = pr_load_client_entry.get()
            if pr_client_id:
                ...
                client_det = session.query(Client).filter_by(client_id = pr_client_id).first()
                ...
                messagebox.showerror("Load client", message="Ensure you input correct client ID for loading..,")
        ...
        #create a load entry
        pr_load_client_entry = tk.Entry(project_update_frame, width=40)
        pr_load_client_entry.grid(row=2, column=0, pady=20)
        #create load button
        pr_load_client_button = tk.Button(project_update_frame, text="Load details", command=load_client)
        pr_load_client_button.grid(row=2, column=1)
    ...
    def user_account_setting():
        from new_base_model import Base, Employee, User
        from new_session import setup_db
        ...
        def clear_user():
            ...
            user_fullname_entry.delete(0, tk.END)
        ...
        def load_emp_user():
            emp_user_id = load_user_entry.get()

            if not emp_user_id:
                messagebox.showerror(title="User load error", message="Make sure to enter correct employee Id")
            elif emp_user_id:
                emp = session.query(Employee).filter_by(employee_id = emp_user_id).one_or_none()

                clear_user()
                if emp:
                    user_fullname_entry.insert(0, emp.employee_name)
                    user_id_entry.insert(0, emp.employee_id)
                else:
                    messagebox.showerror(title="User error", message="Employee ID may be wrong or doesn't exist")
            else:
                messagebox.showerror(title='User id error', message="make sure to enter an existing employee id")
            session.close()
            
        def create_user():
            us_fullname = user_fullname_entry.get()
            username = username_entry.get()
            user_id = user_id_entry.get()
            passwd = password_entry.get()
            acc_level = accesslevel_combo.get()
            ...
            try:
                messagebox.askyesno(title="saving to storage", message="Are you sure you want to save data")
                session.add(user)
                session.commit()
                messagebox.showinfo(title="User account", message="created user account successfully")
                clear_user()
            except Exception as e:
                messagebox.showerror(title="Failure", message="Something went wrong while tring to save user account info, please try again")
                messagebox.showerror(title="Developer info", message=f"emp_id_entry")
            finally:
                session.close()
        ...
        tk.Label(user_account_frame1, text="Username:").grid(row=3, column=0,pady=3)
        ...
        load_user_entry = tk.Entry(user_account_frame1, width=40)
        load_user_entry.grid(row=1, column=1,pady=3)

        load_user_button = tk.Button(user_account_frame1, text="Load user", command=load_emp_user)
        load_user_button.grid(row=1, column=2,padx=3)

        clear_user_button = tk.Button(user_account_frame1, text="Clear", command=clear_user)
        clear_user_button.grid(row=6, column=2,padx=3)

        create_user_button = tk.Button(user_account_frame1, text="Create and save", command=create_user)
        create_user_button.grid(row=6, column=4,pady=3)
        ...
        accesslevel_combo.grid(row=6,column=1)
    ...
    def company_finances():
        from counters import transaction_id
        from new_base_model import Base, Payment
        from new_session import setup_db
        ...
        def process_data():
            ...
            trans_id = transaction_id_entry.get()
            deposit_amount = amount_entry.get()
            deposit_date = datetime.now()

            if trans_id and deposit_amount and deposit_date:
                other_transaction = Payment(transaction_id=trans_id,
                                            total_paid=deposit_amount,
                                            processed_date=deposit_date)
                
                try:
                    session.add(other_transaction)
                    session.commit()
                    messagebox.showinfo(title= "Company finance", message="Processed and saved successfully")
                    transaction_id_entry.delete(0, tk.END)
                    amount_entry.delete(0, tk.END)
                    transaction_id_entry.insert(0, transaction_id())
                except Exception as e:
                    session.rollback()
                    messagebox.showerror(title="Record Error", message="Something went wrong while processing data")
                finally:
                    session.close()
                
               
            else:
                messagebox.showerror(title="Transaction from other activities", message="Something went wrong")
        ...
        process_data_button = tk.Button(other_assets_frame1, text="Process data", command=process_data)
        ...
        for item in other_activities_table.get_children():
            other_activities_table.delete(item)
        records = session.query(Payment).all()
        for record in records:
            other_activities_table.insert("", "end", values=(record.transaction_id, record.total_paid, record.processed_date))
        


        if trans_id and deposit_amount and deposit_date:
            other_activities_table.insert("", "end", values=(trans_id, deposit_amount, deposit_date))
